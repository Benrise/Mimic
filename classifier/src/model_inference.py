...
def classify_text(incoming_message: str, all_messages: list, participant_index: int) -> float:
    classifier = load_classifier()
    conversation_text = format_conversation(all_messages)
    
    logger.info(f"Conversation to classify: {conversation_text}")
    
    result = classifier.predict(incoming_message, conversation_text, participant_index)
